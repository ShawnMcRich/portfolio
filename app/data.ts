...
export const projects: Record<ProjectSlug, Project> = {
  apex: {
    ...
    northStar: t("A useful analysis is one that helps a user understand the setup, the risk, and the counter-case quickly enough to make a calmer decision.", "اگر کاربر موقعیت، ریسک و احتمال اشتباه بودن تحلیل را سریع بفهمد و آرام‌تر تصمیم بگیرد، یعنی تحلیل مفید بوده است."),
    constraints: [
    ...
    phases: [
      ...
      { title: t("Design for the decision", "طراحی برای لحظه تصمیم"), body: t("Outputs lead with a verdict, confidence, why-now context, key levels, invalidation, and the strongest counter-case.", "کاربر اول نتیجه و میزان اطمینان را می‌بیند؛ بعد دلیل تحلیل، سطوح مهم، نقطه بی‌اعتبار شدن آن و قوی‌ترین سناریوی مخالف را."), status: "live" },
      { title: t("Monetize and retain", "درآمدزایی و بازگشت کاربر"), body: t("I introduced prepaid credits, a free trial, simplified analysis modes, daily grants, and targeted win-back communication.", "اعتبار پیش‌پرداخت، دوره آزمایشی، دو حالت ساده تحلیل، اعتبار روزانه و پیام‌های هدفمند برای بازگشت کاربر را طراحی و اجرا کردم."), status: "live" },
    ],
    decisions: [
      ...
      { title: t("Two modes instead of a wall of controls", "دو حالت روشن به‌جای تنظیمات زیاد"), body: t("The analysis builder was simplified to Standard and Forecast. Complexity still exists in the engine, but no longer becomes the user's burden.", "فرایند ساخت تحلیل را به دو حالت استاندارد و پیش‌بینی رساندم. موتور هنوز پیچیده است، اما کاربر مجبور نیست با آن پیچیدگی درگیر شود."), status: "live" },
      { title: t("Reserve inference for moments that create value", "LLM فقط جایی که ارزش می‌سازد"), body: t("Always-on market content uses deterministic language generation; deeper paid analyses use the LLM. This removed about 80 redundant daily calls.", "محتوای روزانه بازار بدون LLM تولید می‌شود و فقط تحلیل‌های عمیق و پولی سراغ مدل می‌روند. با همین تصمیم حدود ۸۰ درخواست تکراری روزانه حذف شد."), status: "shipped" },
      { title: t("Build for emotional pressure, too", "طراحی برای لحظه‌های احساسی"), body: t("The Panic Button reframes an open position through risk, invalidation, and evidence when a user is most likely to act impulsively.", "دکمه Panic درست زمانی به کار می‌آید که معامله‌گر تحت فشار است؛ موقعیت باز را دوباره با ریسک، شواهد و نقطه بی‌اعتبار شدن تحلیل جلوی او می‌گذارد."), status: "live" },
    ],
    architecture: [
    ...
    evidence: [
      ...
      { kind: t("Repository", "مخزن کد"), title: t("Product decisions are visible in the system", "تصمیم‌های محصول در خود سیستم دیده می‌شوند"), body: t("The codebase contains the analysis engine, provider-aware AI service, usage logging, pricing, retention tasks, and explicit TODOs when a promise is not yet true.", "در کد می‌شود موتور تحلیل، سرویس چندارائه‌دهنده AI، ثبت مصرف، قیمت‌گذاری و سازوکارهای بازگشت کاربر را دید. قابلیت‌هایی که هنوز ساخته نشده‌اند هم صریحاً به‌عنوان کار آینده ثبت شده‌اند.") },
      { kind: t("Operations", "عملیات"), title: t("A product, not a prototype", "یک محصول واقعی، نه نمونه نمایشی"), body: t("FastAPI, PostgreSQL, Celery, Redis, Docker, Nginx, and Cloudflare support live delivery and paid usage.", "FastAPI، PostgreSQL، Celery، Redis، Docker، Nginx و Cloudflare زیرساخت استفاده واقعی و پرداخت را فراهم می‌کنند.") },
    ],
  ...
  vibe: {
    ...
    decisions: [
      ...
      { title: t("AI suggests; the user decides", "AI پیشنهاد می‌دهد؛ کاربر تصمیم می‌گیرد"), body: t("If review tone and stars differ by more than one, Vibe explains a suggested rating. The original rating never changes without explicit consent.", "اگر لحن متن با امتیاز بیشتر از یک ستاره اختلاف داشته باشد، Vibe پیشنهادش را همراه با دلیل نشان می‌دهد. بدون تأیید خود کاربر هیچ امتیازی تغییر نمی‌کند."), status: "shipped" },
      { title: t("Identity is evidence, not a verdict", "هویت یک نشانه است، نه حکم نهایی"), body: t("KYC is optional and clearly explained. Credibility also considers contribution history and suspicious patterns so privacy is not the price of being heard.", "KYC اختیاری است و فایده‌اش شفاف توضیح داده می‌شود. اعتبار نظر فقط به مدرک هویتی وابسته نیست؛ سابقه فعالیت و الگوهای مشکوک هم بررسی می‌شوند تا حریم خصوصی هزینه شنیده شدن نباشد."), status: "designed" },
      { title: t("Protect truth in both directions", "حفاظت از حق هر دو طرف"), body: t("The fairness constitution protects genuine reviews, lets businesses challenge fabrication with private evidence, keeps paid tiers away from moderation outcomes, and provides an appeal path.", "قواعد رسیدگی هم از نظر واقعی محافظت می‌کند و هم به کسب‌وکار اجازه می‌دهد با مدرک خصوصی به نظر جعلی اعتراض کند. پول هم هیچ نقشی در نتیجه رسیدگی ندارد و برای تصمیم‌ها راه اعتراض وجود دارد."), status: "designed" },
    ],
    architecture: [
    ...
    evidence: [
      ...
      { kind: t("Delivery", "اجرا"), title: t("Traceable cross-team work", "مسیر کار بین تیم‌ها قابل ردیابی است"), body: t("Jira planning, Git history, backend handoffs, QA tracking, and acceptance criteria connect the idea to implementation and retesting.", "برنامه‌های Jira، تاریخچه Git، اسناد تحویل به بک‌اند، پیگیری QA و شرایط پذیرش نشان می‌دهند هر ایده چطور به اجرا و تست دوباره رسیده است.") },
      { kind: t("Governance", "قواعد محصول"), title: t("Fairness and moderation artifacts", "قواعد انصاف و رسیدگی"), body: t("The product includes a written fairness constitution, credibility model, moderation lifecycle, and explicit boundaries between current and proposed systems.", "برای انصاف، اعتبار نظر و چرخه رسیدگی سند مشخص نوشته‌ایم و مرز بین سیستم فعلی و ایده‌های آینده هم روشن است.") },
    ],
  ...
  "first-choice": {
    ...
    phases: [
      { title: t("Model the operation", "اول خود کسب‌وکار را مدل کردم"), body: t("I mapped public listings, staff dossiers, owner submissions, buyer requests, valuation, leads, and international enquiries into one domain model.", "فایل عمومی، پرونده داخلی، سپردن ملک، درخواست خریدار، ارزیابی، سرنخ و درخواست بین‌المللی را در یک مدل واحد کنار هم قرار دادم."), status: "live" },
      { title: t("Design each language as a product", "هر زبان را جداگانه طراحی کردم"), body: t("The Persian experience supports the local market; English and Arabic emphasize relocation, privacy, and assisted service rather than cloning every page.", "نسخه فارسی روی بازار محلی تمرکز دارد؛ نسخه انگلیسی و عربی بیشتر روی اسکان، حریم خصوصی و خدمات همراه. قرار نبود یک صفحه را سه بار کپی و ترجمه کنیم."), status: "live" },
      { title: t("Create the internal operating surface", "ساخت ابزار داخلی تیم"), body: t("A Persian-first admin lets staff manage properties, people, leads, submissions, media, and private dossiers without developer help.", "یک پنل فارسی ساختم تا کارکنان بدون کمک توسعه‌دهنده بتوانند فایل‌ها، اشخاص، سرنخ‌ها، درخواست‌ها، تصاویر و پرونده‌های خصوصی را مدیریت کنند."), status: "live" },
      { title: t("Build durable discovery", "ساخت ورودی پایدار"), body: t("Search, neighborhood pages, structured data, guides, FAQs, IndexNow, and clean sitemaps create useful entry points beyond paid traffic.", "جست‌وجو، صفحه‌های محله، داده ساختاریافته، راهنماها، پرسش‌های پرتکرار، IndexNow و sitemap تمیز کمک می‌کنند ورودی سایت فقط به تبلیغات وابسته نباشد."), status: "live" },
    ],
    decisions: [
  ...
  mrm: {
    ...
    decisions: [
      { title: t("Familiar interaction over novelty", "رابط آشنا، نه طراحی نمایشی"), body: t("The interface follows established messenger behavior because pressured users needed confidence, not a new mental model.", "رابط را شبیه پیام‌رسان‌های آشنا طراحی کردم؛ در آن شرایط کسی نباید برای فرستادن یک پیام، الگوی تازه‌ای یاد می‌گرفت."), status: "live" },
      ...
      { title: t("Offline-ready, not offline-fiction", "تاب‌آور در قطعی، نه آفلاین خیالی"), body: t("Local caching and pending queues handle short interruptions; the product does not pretend real-time communication can work with no reachable network path at all.", "کش محلی و صف پیام، قطع‌و‌وصل کوتاه را پوشش می‌دهند. اما صادقانه، ارتباط لحظه‌ای وقتی هیچ مسیر شبکه‌ای وجود ندارد ممکن نیست."), status: "live" },
    ...
    evidence: [
      ...
      { kind: t("Boundary", "مرزبندی"), title: t("Claims match the architecture", "ادعاها از معماری جلوتر نمی‌روند"), body: t("The project describes where it is resilient and where it still needs a reachable internal network and independent security review.", "شفاف گفته‌ام سیستم کجا تاب‌آور است و کجا هنوز به شبکه داخلی در دسترس و بررسی امنیتی مستقل نیاز دارد.") },
    ],
