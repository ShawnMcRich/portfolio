...
export const projects: Record<ProjectSlug, Project> = {
  apex: {
    ...
    role: t("AI Product Manager · Product design and full-stack delivery", "مدیر محصول هوش مصنوعی · طراحی محصول و اجرای Full-stack"),
    team: t("Independent product with production infrastructure and paid users", "محصول مستقل با زیرساخت عملیاتی و کاربر پرداخت‌کننده"),
  ...
  vibe: {
    ...
    deck: t("I shaped Vibe's roadmap, designed every customer and business flow, built the frontend from zero, and worked with the team on the trust and moderation systems needed before launch.", "نقشه راه Vibe را چیدم، تمام مسیرهای کاربر و کسب‌وکار را طراحی کردم، فرانت‌اند را از صفر ساختم و همراه تیم سراغ سیستم‌های اعتماد و رسیدگی لازم برای لانچ رفتم."),
    role: t("Technical Product Lead · Senior Frontend Engineer", "رهبر فنی محصول · توسعه‌دهنده ارشد فرانت‌اند"),
  ...
  "first-choice": {
    ...
    role: t("Technical Product Lead · Full-stack Developer", "رهبر فنی محصول · توسعه‌دهنده Full-stack"),
    ...
    metrics: [
      { value: t("~1,000/day", "حدود ۱۰۰۰ در روز"), label: t("property records processed per city", "فایل پردازش‌شده برای هر شهر") },
      ...
      { value: t("9.6s → 2.8s", "از ۹.۶ به ۲.۸ ثانیه"), label: t("mobile LCP", "LCP موبایل") },
    ],
    ...
    phases: [
      ...
      { title: t("Automate property ingestion", "خودکارسازی دریافت فایل ملک"), body: t("A scheduled pipeline collects roughly 1,000 new property records per city each day, normalizes and deduplicates them, rejects or flags unsuitable records, and makes approved inventory available to the public site and staff panel.", "یک فرایند زمان‌بندی‌شده روزانه حدود ۱۰۰۰ فایل جدید برای هر شهر دریافت می‌کند، داده‌ها را یکدست می‌کند، موارد تکراری یا نامناسب را حذف یا علامت‌گذاری می‌کند و فایل‌های تأییدشده را در سایت و پنل کارکنان قرار می‌دهد."), status: "live" },
    ...
    architecture: [
      ...
      { title: t("Data ingestion", "دریافت و پالایش داده"), body: t("Scheduled collection, normalization, duplicate handling, quality flags, approval, and controlled publishing.", "دریافت زمان‌بندی‌شده، یکدست‌سازی، تشخیص تکراری‌ها، علامت‌گذاری کیفیت، تأیید و انتشار کنترل‌شده.") },
    ...
    scope: [
      ...
      t("Automated property ingestion, cleaning, quality rules, approval, and publishing", "دریافت خودکار فایل‌های ملک، پاک‌سازی داده، قواعد کیفیت، تأیید و انتشار"),
    ...
    evidence: [
      ...
      { kind: t("Measured delivery", "نتیجه قابل اندازه‌گیری"), title: t("Data and performance results", "نتیجه قابل بررسی در داده و سرعت"), body: t("The operating system processes roughly 1,000 incoming property records per city each day. Recorded delivery work also moved mobile Lighthouse performance from 53 to 95 and LCP from 9.6 to 2.8 seconds.", "سیستم عملیاتی روزانه حدود ۱۰۰۰ فایل ورودی برای هر شهر را پردازش می‌کند. در بهینه‌سازی موبایل هم امتیاز Lighthouse از ۵۳ به ۹۵ و LCP از ۹.۶ به ۲.۸ ثانیه رسیده است." ) },
    ],
    outcome: t("First Choice is now a live operating platform connecting automated property ingestion, public discovery, multilingual enquiries, controlled publishing, and private staff workflows.", "First Choice اکنون یک پلتفرم فعال است که دریافت خودکار فایل‌های ملک، جست‌وجوی عمومی، درخواست‌های چندزبانه، انتشار کنترل‌شده و فرایندهای خصوصی کارکنان را در یک سیستم قرار می‌دهد."),
  ...
  hosseintalab: {
    ...
    role: t("Product Manager · Product design and frontend delivery", "مدیر محصول · طراحی محصول و اجرای فرانت‌اند"),
  ...
  mrm: {
    ...
    role: t("Product Owner · Full-stack Engineer", "مالک محصول · توسعه‌دهنده Full-stack"),
